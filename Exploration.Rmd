...
> 1. Remove blank-demographic entries (COMPLETE)
  - Our research motivation specifically calls out age, race, income, and gender. Let's purge age-blank entries first and see what remains. Then, second, wealth-blank entries. Third, race-blank. Last, gender-blank (maybe leave them).
  - Filter results further, using only the top four race/ethnicity responses.
  - Note data set size before and after.
  
> 2. Separate distributions for each question by subsets (COMPLETE)
  - House price vs. age for each age group
  - House price vs. gender for each gender
  - House price vs. wealth by each wealth group
  - House price vs. race/ethnicity
>  
  - Night safety vs. age for each age group
  - Night safety vs. gender for each gender
  - Night safety vs. wealth by each wealth group
  - Night safety vs. race/ethnicity
(Note, this will be done after demographic distributions, so scroll down a bit)
  
> 2.5. Include renters and students in demographic breakdown (COMPLETE)

> 3. Find Somerville population demographics to compare our sample demographics to
...
renter = demographicssomerville2017[6]
